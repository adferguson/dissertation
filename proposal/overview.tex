...
(\S\ref{sec:messages}-\ref{sec:FullSystem}).
...
\sys addresses two key challenges. First, it provides a flexible
...
\sys uses \emph{shares} to limit the authority of principals, a
concept we borrow and extend from~\cite{Ferguson:2012a}.  A share
...
We use Hierarchical Flow Tables (HFTs), introduced by Ferguson,
\etal~\cite{Ferguson:2012b}, to materialize \sys's policy tree.  HFTs
...
of \sys's operators and its use of the HFT compiler in
\xref{sec:conflicts}.
...
%\begin{small}
%\begin{Verbatim}
%1 root: NewShare aliceBW for (user=Alice)
%  [reserve <= 10Mb] on rootShare.
%2 root: Grant aliceBW to Alice.
%3 Alice: reserve(user=Alice,dstPort=80) = 8Mb
%  on aliceBW from +20min to +30min.
%4 root: NewShare bobAC for (dstHost=10.0.0.2)
%  [deny = True] on rootShare.
%5 root: Grant bobAC to Bob.
%6 Bob: deny(dstHost=10.0.0.2, srcHost=10.0.0.3)
%  on bobAC from now to +5min.
%7 Bob: deny(dstHost=10.0.0.4, srcHost=10.0.0.3)
%  on bobAC.
%\end{Verbatim}
%\end{small}
%\end{boxedminipage}
%\caption{Sample interaction between two principals and \sys.}
%\end{figure}
