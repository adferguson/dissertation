...
\item {\bf Virtualization} -- Virtualizing the network for SDN applications consists of two components:
	\begin{itemize}
	\item {\bf Indirection} -- Writing a policy on an abstract view of the network (\eg, ``one big switch'', or a particular slice~\cite{slicing})
...
federation~\cite{hyperflow}, hierarchy~\cite{kandoo,logical_xbars}, and scalable state storage.
...
resources, and are the right place to intelligently optimize their use~\cite{devoflow,difane,nanxi,palette}. Ideally, such platforms
...
SDN is ongoing~\cite{reitblatt,ratul,of.cpp,sdn-transactions}.
\item {\bf Queries} -- Measuring network state by querying the data-plane is a key component of
many network applications~\cite{hedera,etc}. SDN control platforms will need intelligence to answer
queries from multiple applications without overwhelming limited network hardware~\cite{oflops}.
